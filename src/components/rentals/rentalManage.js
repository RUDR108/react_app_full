...
class RentalManage extends React.Component{
    ...
    actions.getUserRentals().then(
        userRentals=>this.setState({userRentals,errors:[],isFetching:false}),
        errors=>this.setState({errors,isFetching:false})
        )
    }

    renderRentalCards(rentals) {
        return rentals.map((rental, index) => <RentalManageCard modal={<RentalManageModal bookings={rental.bookings} />} key={index} rental={rental} deleteRentalCb={this.deleteRental} rentalIndex={index} />);
    }
...
export default connect()(RentalManage)
